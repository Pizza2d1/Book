...
class Read(Variables):      #Reads the text file and allows the user to decide which iteration to read or to read all of them
    ...
    def read(self):                 #Reads
        ...
        if user_input == '':                        #Checks to see whether the user properly inputted ANYTHING
            return
        readlist = []
        if user_input.isdigit():                    #If the user input is a number then it selects the iteration that the user selected to read, doesn't exclude deleted iterations even with the setting set to false
            if int(user_input) > self.number_of_iterations:
                ...
                if new_user_input == 'y' or new_user_input == 'yes' or new_user_input == '1':
                    ...
                    return
        elif user_input in ['a', 'all', 'ALL']:     #Will read all iterations of the book in sequential/chronological order
            codescheck = []
